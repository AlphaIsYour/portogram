...
const prisma_1 = require("../app/generated/prisma"); // <-- PASTIKAN PATH BENAR
const fas = __importStar(require("@fortawesome/free-solid-svg-icons"));
const fab = __importStar(require("@fortawesome/free-brands-svg-icons"));
...
try {
    ...
    function iconToString(icon) {
        return icon === null || icon === void 0 ? void 0 : icon.iconName;
    }
    ...
    function parseCollaborators(collabs) {
    ...
    function parseFloatRating(ratingStr) {
        if (!ratingStr)
            return undefined;
    ...
    async function main() {
        ...
        const CLEAN_DATABASE = true; // <-- SET KE true JIKA INGIN BERSIHKAN DATABASE DULU!
        if (CLEAN_DATABASE) {
            console.warn("WARNING: Cleaning existing database...");
            // Hapus junction M2M dulu
            await prisma.projectTestimonial.deleteMany();
            console.log("- Deleted ProjectTestimonial");
            await prisma.skill.updateMany({ data: { projects: { set: [] } } }); // Disconnect M2M Skill-Project
            console.log("- Disconnected Skills from Projects");
            // Hapus children 1-M
            ...
            console.log("- Deleted main dependent tables");
            // Terakhir User
        ...
        const user = await prisma.user.upsert({
            ...
            update: {
                ...
                phone: (_a = contacts.find((c) => c.type === "Phone")) === null || _a === void 0 ? void 0 : _a.value,
                location: (_b = contacts.find((c) => c.type === "Location")) === null || _b === void 0 ? void 0 : _b.value,
                bio: "Passionate Full Stack Developer specializing in modern web technologies.", // Ganti bio
                website: (_c = contacts.find((c) => c.type === "Website")) === null || _c === void 0 ? void 0 : _c.value,
            ...
            create: {
                ...
                phone: (_d = contacts.find((c) => c.type === "Phone")) === null || _d === void 0 ? void 0 : _d.value,
                location: (_e = contacts.find((c) => c.type === "Location")) === null || _e === void 0 ? void 0 : _e.value,
                bio: "Passionate Full Stack Developer specializing in modern web technologies.", // Ganti bio
                website: (_f = contacts.find((c) => c.type === "Website")) === null || _f === void 0 ? void 0 : _f.value,
                avatar: "/images/avatar-placeholder.png", // Ganti avatar
            },
        });
        ...
        for (const testimonial of testimonials) {
            ...
            if (!parsedDate) {
                console.warn(`Skipping testimonial "${testimonial.name}" due to invalid date.`);
                continue;
            }
        ...
        for (const project of projects) {
            ...
            if (!startDate) {
                console.warn(`Skipping project "${project.title}" due to invalid start date.`);
                continue;
            }
            try {
                const createdProject = await prisma.project.create({
                    data: {
                        ...
                        longDescription: (_g = project.longDescription) !== null && _g !== void 0 ? _g : "",
                        ...
                        branches: safeParseInt((_h = project.stats) === null || _h === void 0 ? void 0 : _h.branches),
                        stars: safeParseInt((_j = project.stats) === null || _j === void 0 ? void 0 : _j.stars),
                        views: safeParseInt((_k = project.stats) === null || _k === void 0 ? void 0 : _k.views),
                        forks: safeParseInt((_l = project.stats) === null || _l === void 0 ? void 0 : _l.forks),
                        issues: safeParseInt((_m = project.stats) === null || _m === void 0 ? void 0 : _m.issues),
                        commits: safeParseInt((_o = project.stats) === null || _o === void 0 ? void 0 : _o.commits),
                ...
                if (error instanceof prisma_1.Prisma.PrismaClientKnownRequestError &&
                    error.code === "P2002") {
                    console.warn(`Project slug "${project.id}" already exists. Fetching existing ID...`);
                    const existing = await prisma.project.findUnique({
                        where: { slug: project.id },
                        select: { id: true },
                    });
                    if (existing)
                        createdProjectsMap.set(project.id, { id: existing.id });
                    else
                        console.error(`Could not fetch existing project for slug "${project.id}" despite P2002 error.`);
                }
                else {
        ...
        for (const cert of certifications) {
            ...
            if (!date) {
                console.warn(`Skipping certification "${cert.name}" due to invalid date.`);
                continue;
            }
        ...
        for (const achievement of achievements) {
            ...
            if (!date) {
                console.warn(`Skipping achievement "${achievement.title}" due to invalid date.`);
                continue;
            }
            ...
            try {
                await prisma.blogPost.create({
                    data: {
                        ...
                        content: (_p = post.content) !== null && _p !== void 0 ? _p : "",
                        date: date,
                        readTime: (_q = parseReadTime(post.readTime)) !== null && _q !== void 0 ? _q : 5,
                        image: post.image,
                        ...
                        author: (_r = post.author) !== null && _r !== void 0 ? _r : user.name,
                        tags: post.tags
                ...
                if (error instanceof prisma_1.Prisma.PrismaClientKnownRequestError &&
                    error.code === "P2002") {
                    console.warn(`Blog post slug "${post.slug}" already exists. Skipping.`);
                }
                else {
        ...
        for (const contact of contacts) {
            ...
            if (contact.type === "Email" || contact.type === "Website") {
                console.log(`Skipping contact type "${contact.type}" as it might be on the User model.`);
                continue;
            }
        ...
        if (devStats) {
            ...
            await prisma.devStats.upsert({
                ...
                update: {
                    ...
                    rating: (_s = parseFloatRating(devStats.rating)) !== null && _s !== void 0 ? _s : 0.0,
                ...
                create: {
                    ...
                    stars: safeParseInt(devStats.stars),
                    contributions: safeParseInt(devStats.contributions),
                    rating: (_t = parseFloatRating(devStats.rating)) !== null && _t !== void 0 ? _t : 0.0,
                    followers: safeParseInt(devStats.followers),
                    following: safeParseInt(devStats.following),
                    streak: safeParseInt(devStats.streak),
                    commits: safeParseInt(devStats.commits),
            ...
            if (skillData.projectsUsedIn && skillData.projectsUsedIn.length > 0) {
                ...
                for (const projectIdentifier of skillData.projectsUsedIn) {
                    ...
                    if (foundProject) {
                        ...
                        console.warn(`Skill "${skillName}" - Project slug "${projectSlugOrTitle}" not found directly. Trying by title...`);
                        const projectByTitle = await prisma.project.findFirst({
                            where: { title: projectSlugOrTitle, userId: user.id },
                            select: { id: true },
                        });
                        if (projectByTitle) {
                            console.warn(`   Found project by title: "${projectSlugOrTitle}" (ID: ${projectByTitle.id})`);
                            projectConnections.push({ id: projectByTitle.id });
                        }
                        else {
                            console.error(`   ERROR: Skill "${skillName}" wants to connect to non-existent project (slug/title): "${projectSlugOrTitle}". Skipping connection.`);
                        }
                    }
                }
                if (projectConnections.length > 0) {
                    try {
                        ...
                        console.error(`Error connecting Skill "${skillName}" to projects:`, m2mError);
                    }
                }
            }
        }
        ...
        for (const [testimonialDynamicId, testimonialData,] of createdTestimonialsMap.entries()) {
            if (testimonialData.projectLink) {
                ...
                if (!relatedProject) {
                    console.warn(`Testimonial "${testimonialData.id}" - Project slug "${projectIdentifier}" not found directly. Trying by title...`);
                    const projectByTitle = await prisma.project.findFirst({
                        where: { title: projectIdentifier, userId: user.id },
                        select: { id: true },
                    });
                    if (projectByTitle) {
                        console.warn(`   Found project by title: "${projectIdentifier}" (ID: ${projectByTitle.id})`);
                ...
                if (relatedProject) {
                    try {
                        ...
                        if (error instanceof prisma_1.Prisma.PrismaClientKnownRequestError &&
                            ...
                            console.error(`Error creating ProjectTestimonial link for Testimonial DynID ${testimonialDynamicId} & Project ${projectIdentifier}:`, error);
                        }
                    }
                }
                else {
                    console.error(`   ERROR: Could not find project (slug/title) '${projectIdentifier}' referenced by testimonial (DynID: ${testimonialDynamicId}). Skipping connection.`);
                }
            }
        }
        console.log(`Created ${projectTestimonialLinks} Project-Testimonial links.`);
